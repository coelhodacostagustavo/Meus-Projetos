...
function atualizarRelogios() {
    ...
    locais.forEach(local => {
        ...
        const horaLocal = new Date(agora.setHours(agora.getUTCHours() + local.fusoHorario));

        const dataFormatada = horaLocal.toLocaleDateString('pt-BR');

        const horaFormatada = horaLocal.toLocaleDateString('pt-BR', {hour: '2-digit', minute: '2-digit', second: '2-digit'});
